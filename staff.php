...
const staffSharePanel = document.getElementById('staffSharePanel');
const staffShareToggle = document.getElementById('staffShareToggle');
const staffShareMenu = document.getElementById('staffShareMenu');
const staffShareCopyBtn = document.getElementById('staffShareCopyBtn');
const staffShareNativeBtn = document.getElementById('staffShareNativeBtn');
const staffShareFeedback = document.getElementById('staffShareFeedback');

if (staffSharePanel) {
    const shareUrl = staffSharePanel.dataset.shareUrl || window.location.href;
    const shareText = staffSharePanel.dataset.shareText || 'Lihat profil staff ini di ALVETRIX';

    const setShareFeedback = (message, isError = false) => {
        if (!staffShareFeedback) return;
        staffShareFeedback.textContent = message;
        staffShareFeedback.classList.toggle('error', isError);
    };
    const openShareMenu = () => {
        staffSharePanel.classList.add('open');
        if (staffShareToggle) {
            staffShareToggle.setAttribute('aria-expanded', 'true');
        }
    };
    const closeShareMenu = () => {
        staffSharePanel.classList.remove('open');
        if (staffShareToggle) {
            staffShareToggle.setAttribute('aria-expanded', 'false');
        }
    };
    ...
    if (staffShareToggle && staffShareMenu) {
        staffShareToggle.addEventListener('click', (event) => {
            event.stopPropagation();
            const isOpen = staffSharePanel.classList.contains('open');
            if (isOpen) {
                closeShareMenu();
            } else {
                openShareMenu();
        ...
        staffShareMenu.addEventListener('click', (event) => {
            event.stopPropagation();
        });

        document.addEventListener('click', (event) => {
            if (!staffSharePanel.contains(event.target)) {
                closeShareMenu();
            }
        });
    ...
    if (staffShareCopyBtn) {
        staffShareCopyBtn.addEventListener('click', () => {
            const handleCopySuccess = () => {
                staffShareCopyBtn.classList.add('copied');
                setShareFeedback('Tautan profil berhasil disalin.');
                setTimeout(() => {
                    staffShareCopyBtn.classList.remove('copied');
                    setShareFeedback('');
                }, 1600);
            };
    ...
    if (staffShareNativeBtn && navigator.share) {
        staffShareNativeBtn.style.display = 'inline-flex';
        staffShareNativeBtn.addEventListener('click', () => {
            navigator.share({
                title: 'Profil Staff ALVETRIX',
                text: shareText,
                url: shareUrl
            }).then(() => {
